...
- **2026-09-22 — Teams roster sync reports in:** a Power Automate flow keeps the SCEN
  Communication team's private channels in step with a roster workbook, and posts what it
  read to `/api/v1/team-roster/syncs` with an API token after every run. This application
  still calls nothing outside itself: it holds one dated reading, never a connection to
  Teams, and answers "we have never been told" differently from "nobody is missing". A
  cohort names its channel (`teamsChannel`, empty for every cohort until somebody sets it),
  and a student's address is derived as `studentId` lowercased at the university domain —
  the workbook carries no other identifier. Readings are kept 50 deep so a wrong warning
  can be traced to the roster or the cohort. Do not add a live Teams or Graph dependency
  here without an approved scope.

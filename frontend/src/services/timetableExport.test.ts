  ...
  scopes: [
    {
      ...
      courses: [{ id: "td-math", code: "MATH001", name: "Pre-calculus 1", component: "TD", ue: "", parentCrn: "24226" }],
      groups: [
...
describe("the request sheets", () => {
  it("write one row per section in the workbook's columns, with the retired ones marked", () => {
    ...
    expect(sheet.rows[0]).toMatchObject({
      courseName: "Pre-calculus 1 G.1-TD", crn: "23223", parentCrn: "24226", subject: "MATH", courseNumber: "001",
  ...
  it("is the workbook: a sheet per cohort with the header on row 4, the CRN table, teacher hours, the list", async () => {
    const sheets = requestSheets(buildCards([FYS], termName), "term-1", "Semester 1", () => "FYS", nameOf);
    ...
    expect(sheet.getRow(5).values).toEqual([undefined, "Pre-calculus 1 G.1-TD", "FYS", undefined, 23223, 24226, "MATH", "001", 50, "TD", undefined, "Samar Ghantous", undefined, undefined, "Should NOT be in parallel with G.2", "2 sessions - weeks 2 to 14", 1.5, 33, "Mutualized with Maths"]);

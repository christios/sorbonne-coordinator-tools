...
describe("the shape of a semester", () => {
  it("counts what each set holds, and puts the department's sets last", () => {
    const own = scope({ id: "s1", code: "TD", courses: [{ id: "c1", code: "MATH-001", name: "", component: "" }], groups: [group("g1", "1", 30), group("g2", "2", 28)] });
    const shared = scope({ id: "s2", code: "LANG", openToAll: true, groups: [group("g3", "A1", 24)], courses: [{ id: "c2", code: "SCEN-101", name: "", component: "" }] });
  ...
  it("names the two faults that actually break a fill, and the two that are merely idle", () => {
    ...
    const adrift = scope({ kind: "nested", parentScopeId: "p", groups: [group("g", "1A", 0, "gone")], courses: [{ id: "c", code: "X", name: "", component: "" }] });
    ...
    expect(troubleWith(scope({ courses: [{ id: "c", code: "X", name: "", component: "" }] }), byId)).toEqual(["no group"]);

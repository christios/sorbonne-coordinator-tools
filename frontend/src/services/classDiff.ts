 ...
 * and the ones that have gone in the same squares. Both, deliberately. A diff that showed
 * only what was lost would leave the reader counting backwards to work out what is left.
 *
 ...
 *
 * Keyed on the missing classes rather than on the section, so an approval lasts exactly
 * as long as the fact it was about: one more class going changes the key, stops the old
 * approval matching, and asks again.
 */
 ...
 */
export function monthsOfDiff(kept: Meeting[], removed: Meeting[]): DiffMonth[] {
  const days = new Map<string, DiffDay>();
  const note = (meeting: Meeting, into: "kept" | "removed") => {
  ...
  for (const day of touched) {
    ...
    for (let date = 1; date <= last.getDate(); date += 1) {
      ...
      cells.push(days.get(on) ?? { day: on, dayOfMonth: date, kept: [], removed: [] });
    }

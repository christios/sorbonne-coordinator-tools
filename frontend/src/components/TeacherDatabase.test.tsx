...
describe("ProfileOverview", () => {
  it("shows profile information until the coordinator chooses to edit it", async () => {
    ...
    fireEvent.change(screen.getByRole("textbox", { name: "Full name" }), {
      target: { value: "Marie Skłodowska-Curie" },
    });
  ...
  it("provides an accessible profile photo placeholder for a teacher", () => {
    ...
    expect(
      screen.getByRole("img", {
        name: "Profile photo placeholder for Marie Curie",
      }),
    ).toBeTruthy();
  });
});
...
describe("RequisitionHistory", () => {
  ...
  it("shows outlined requisition items and filters them by label or academic year", () => {
    ...
    expect(
      screen.getByText("Physics lab").closest("article")?.className,
    ).toContain("border");
  ...
  it("lets the coordinator rename a requisition directly from its history card", async () => {
    ...
    fireEvent.change(
      screen.getByRole("textbox", { name: "Requisition title" }),
      { target: { value: "Semester 1 tutorial" } },
    );
    fireEvent.blur(screen.getByRole("textbox", { name: "Requisition title" }));

    await waitFor(() =>
      expect(onRename).toHaveBeenCalledWith("request-1", "Semester 1 tutorial"),
    );
...
describe("RequisitionDetails", () => {
  it("exposes every job title and the employee type defined by the approved form", () => {
    render(
      <RequisitionDetails
        content={{
          department: "",
    ...
    expect(
      screen.getByRole("textbox", { name: "Hiring department" }),
    ...
    expect(
      screen
        .getByRole("combobox", { name: "Job title" })
        .getAttribute("aria-required"),
    ).toBe("true");
    expect(
      screen
        .getByRole("combobox", { name: "Employee type" })
        .getAttribute("aria-required"),
    ).toBe("true");
    expect(
      screen
        .getByRole("combobox", { name: "Type of class" })
        .getAttribute("aria-required"),
    ).toBe("true");
    expect(
      screen
    ...
    requiredMarkers.forEach((marker) =>
      expect(marker.className).toContain("text-[#a6292f]"),
    );

    fireEvent.click(screen.getByRole("combobox", { name: "Job title" }));
    expect(
...
describe("TeacherRequisitionEditor", () => {
  it("edits the requisition title inline and returns to text when focus leaves", async () => {
    ...
    expect(
      screen.queryByRole("textbox", { name: "Requisition title" }),
    ).toBeNull();
    expect(
      screen.getByRole("button", { name: "Semester 1 tutorial" }),
    ).toBeTruthy();
  });
  ...
  it("autosaves edits and replaces the manual save action with the syllabus-style status", async () => {
    ...
    const academicYear = await screen.findByRole("textbox", {
      name: "Academic year",
    });
    ...
    await waitFor(
      () =>
        expect(teacherService.updateTeacherRequisition).toHaveBeenCalledWith(
          expect.objectContaining({ academicYear: "2027-2028", revision: 1 }),
        ),
      { timeout: 1500 },
    );
    expect(screen.getByText("Saved")).toBeTruthy();
  ...
  it("takes export validation to the missing teaching-load field", async () => {
    ...
    const courseTitle = await screen.findByRole("textbox", {
...
describe("RequisitionReview", () => {
  it("summarises the teacher, request details, load, and export readiness", () => {
    ...
    expect(screen.getByText("Employee type")).toBeTruthy();
    expect(screen.getByText("Part Time Employee")).toBeTruthy();
    ...
    expect(screen.getByText("Employee type").className).toContain(
      "text-[#1f4e79]",
    );
  });
});

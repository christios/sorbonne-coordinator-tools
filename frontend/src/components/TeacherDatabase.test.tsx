...
describe("RequisitionDetails", () => {
  it("exposes every job title and the employee type defined by the approved form", () => {
    ...
    expect(screen.getByRole("combobox", { name: "Employee type" }).getAttribute("aria-required")).toBe("true");
...
describe("RequisitionReview", () => {
  it("summarises the teacher, request details, load, and export readiness", () => {
    ...
    expect(screen.getByText("Employee type").className).toContain("text-[#1f4e79]");
  });
});

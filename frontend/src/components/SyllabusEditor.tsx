...
function SectionForm({ active, draft, editContent, editMetadata, onOpenHistory }: { active: string; draft: Syllabus; editContent: (section: string, value: unknown) => void; editMetadata: (field: "courseTitle" | "courseCode" | "academicYear", value: string) => void; onOpenHistory: (field: HistoryField) => void }) {
  ...
  if (active === "learningOutcomes") return <Section title="Learning outcomes">{text("Programme learning outcomes (one per line)", Array.isArray(section.plos) ? section.plos.join("\n") : "", (value) => editContent(active, { ...section, plos: value.split("\n").filter(Boolean) }), true)}<RowsEditor title="Course learning outcomes and alignment" columns={[["clo", "Course learning outcome"], ["plo", "Aligned PLO"], ["skills", "Graduate skills"]]} rows={(section.clos as Row[]) ?? []} onChange={(clos) => editContent(active, { ...section, clos })} historyPath="learningOutcomes.clos" syllabusId={draft.id} revision={draft.revision} onOpenHistory={onOpenHistory} /></Section>;
  if (active === "schedule") return <Section title="Course schedule"><RowsEditor title="Sessions" columns={[["session", "Session"], ["date", "Date"], ["topic", "Topic"], ["activities", "Pre-class learning / assessments"]]} rows={(content.schedule as Row[]) ?? []} onChange={(schedule) => editContent(active, schedule)} historyPath="schedule" syllabusId={draft.id} revision={draft.revision} onOpenHistory={onOpenHistory} /></Section>;
  if (active === "bibliography") return <Section title="Supplemental bibliographical resources">{text("Books", section.books, (value) => editContent(active, { ...section, books: value }), true)}{text("Websites", section.websites, (value) => editContent(active, { ...section, websites: value }), true)}{text("Journal articles", section.journalArticles, (value) => editContent(active, { ...section, journalArticles: value }), true)}</Section>;
  ...
  if (active === "assessment") return <Section title="Course assessment"><RowsEditor title="Summary of graded learning activities" columns={[["date", "Assessment date"], ["type", "Assessment type"], ["weight", "Weight (%)"], ["clos", "CLOs assessed"], ["ai", "AI policy"]]} rows={(section.items as Row[]) ?? []} onChange={(items) => editContent(active, { ...section, items })} historyPath="assessment.items" syllabusId={draft.id} revision={draft.revision} onOpenHistory={onOpenHistory} /><SelectField label="AI policy" value={stringify(section.aiPolicy) || "AI Prohibited"} onChange={(value) => editContent(active, { ...section, aiPolicy: value })} history={history("AI policy")} options={["AI Prohibited", "AI Permitted as a Support Tool", "Restricted AI Use", "AI Required", "Other (Specify)"]} />{text("Additional instructions regarding AI", section.aiInstructions, (value) => editContent(active, { ...section, aiInstructions: value }), true)}{text("Assessment methodologies", section.methodologies, (value) => editContent(active, { ...section, methodologies: value }), true)}<RowsEditor title="Grading rubrics" columns={[["assignment", "Assessment type"], ["criteria", "Criteria"], ["meets", "Meets expectations"], ["exceeds", "Exceeds expectations"]]} rows={(section.rubrics as Row[]) ?? []} onChange={(rubrics) => editContent(active, { ...section, rubrics })} historyPath="assessment.rubrics" syllabusId={draft.id} revision={draft.revision} onOpenHistory={onOpenHistory} />{text("Late submission policy", section.lateSubmissionPolicy, (value) => editContent(active, { ...section, lateSubmissionPolicy: value }), true)}</Section>;

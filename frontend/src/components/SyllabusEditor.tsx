...
function SectionForm({ active, draft, editContent, editMetadata, onOpenHistory }: { active: string; draft: Syllabus; editContent: (section: string, value: unknown) => void; editMetadata: (field: "courseTitle" | "courseCode" | "academicYear", value: string) => void; onOpenHistory: (field: HistoryField) => void }) {
  ...
  if (active === "identification") return <CourseIdentificationEditor value={section} courseTitle={draft.courseTitle} courseCode={draft.courseCode} academicYear={draft.academicYear} onChange={(identification) => editContent(active, identification)} onMetadataChange={editMetadata} syllabusId={draft.id} revision={draft.revision} onOpenHistory={onOpenHistory} />;
  if (active === "contacts") return <AcademicContactsEditor value={section} onChange={(contacts) => editContent(active, contacts)} syllabusId={draft.id} revision={draft.revision} onOpenHistory={onOpenHistory} />;

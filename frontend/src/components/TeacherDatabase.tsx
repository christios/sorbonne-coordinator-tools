...
export function TeacherDatabase() {
  ...
  const create = useMutation({
    ...
    onSuccess: (teacher) => {
      refreshLibrary();
      setScreen({ view: "profile", teacherId: teacher.id });
    },
  });
  ...
  if (screen.view === "library")
    return (
      <TeacherLibrary
        ...
        isLoading={
          teachers.isLoading || folders.isLoading || teacherTasks.isLoading
        }
        error={
          (
            teachers.error ??
    ...
    <div className="mx-auto max-w-7xl px-1 py-6 sm:px-1.5 lg:px-2">
          ...
          <button
            ...
            onClick={() => {
              if (!showTeacherForm)
                setDraft((current) => ({
                  ...current,
                  taskTemplateIds: taskTemplates.map((template) => template.id),
                }));
      ...
      {showTeacherForm ? (
        <form
          ...
          className="mt-5 grid items-start gap-4 rounded-lg border border-[#cbd5e1] bg-white p-5 md:grid-cols-2"
        >
          ...
          {taskTemplates.map((template) => (
            <label
              key={template.id}
              className="flex items-start gap-2 rounded-md border border-[#d9dee7] p-3 text-sm text-[#344054] md:col-span-2"
            >
              <input
                type="checkbox"
          ...
          <div className="flex gap-3 md:col-span-2">
            <button
              disabled={creating}
              className="rounded-md bg-[#1f4e79] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"
            >
            ...
            <button
              ...
              className="rounded-md border border-[#b7bec8] px-4 py-2 text-sm font-semibold"
            >
              Cancel
            </button>
          </div>
      ...
      {showFolderForm ? (
        <form
          ...
          className="mt-5 flex max-w-lg gap-3 rounded-lg border border-[#cbd5e1] bg-white p-4"
        >
          ...
          <button
            ...
            className="self-end rounded-md bg-[#1f4e79] px-3 py-2 text-sm font-semibold text-white"
          >
            Create
          </button>
        </form>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[230px_minmax(0,1fr)]">
        <aside className="rounded-lg border border-[#d9dee7] bg-white p-2 lg:h-fit">
          <div className="grid grid-cols-2 gap-1 px-1 pb-3 pt-1">
            <button
              type="button"
              onClick={() => setLibraryView("teachers")}
              className={`rounded-md px-3 py-2 text-sm font-semibold ${libraryView === "teachers" ? "bg-[#e8edf3] text-[#1f4e79]" : "text-[#475467] hover:bg-[#f7f8fa]"}`}
            >
              Teachers
            </button>
            <button
              ...
              className={`rounded-md px-3 py-2 text-sm font-semibold ${libraryView === "tasks" ? "bg-[#e8edf3] text-[#1f4e79]" : "text-[#475467] hover:bg-[#f7f8fa]"}`}
            >
              Tasks
            </button>
          </div>
          <p className="px-3 pb-2 pt-1 text-xs font-semibold uppercase text-[#667085]">
            Teachers
          </p>
          <button
            type="button"
            onClick={() => setShowArchived(false)}
            className={`block w-full rounded-md px-3 py-2 text-left text-sm ${!showArchived ? "bg-[#e8edf3] font-semibold text-[#1f4e79]" : "hover:bg-[#f7f8fa]"}`}
          >
          ...
          <button
            ...
            className={`block w-full rounded-md px-3 py-2 text-left text-sm ${showArchived ? "bg-[#e8edf3] font-semibold text-[#1f4e79]" : "hover:bg-[#f7f8fa]"}`}
          >
            Archived ({teachers.length - activeTeachers.length})
          </button>
          <p className="px-3 pb-2 pt-4 text-xs font-semibold uppercase text-[#667085]">
            Folders
          </p>
          <button
            type="button"
            onClick={() => setActiveFolder("all")}
            className={`block w-full rounded-md px-3 py-2 text-left text-sm ${activeFolder === "all" ? "bg-[#e8edf3] font-semibold text-[#1f4e79]" : "hover:bg-[#f7f8fa]"}`}
          >
            All teachers
          </button>
          {activeTeachers.some((teacher) => teacher.folderId === null) ? (
            <button
              type="button"
              onClick={() => setActiveFolder(UNFILED)}
              className={`block w-full rounded-md px-3 py-2 text-left text-sm ${activeFolder === UNFILED ? "bg-[#e8edf3] font-semibold text-[#1f4e79]" : "hover:bg-[#f7f8fa]"}`}
            >
              Unfiled
            </button>
          ) : null}
          {folderTree.map(({ folder, depth }) => (
            ...
            >
              <button
                ...
                className={`min-w-0 flex-1 rounded-md px-3 py-2 text-left text-sm ${activeFolder === folder.id ? "bg-[#e8edf3] font-semibold text-[#1f4e79]" : "hover:bg-[#f7f8fa]"}`}
              >
                {folder.name}
              </button>
              <button
                ...
                className="p-2 text-[#a6292f] disabled:opacity-50"
              >
                <Trash2 size={15} />
              </button>
            </div>
              ...
              <div className="border-b border-[#e5e7eb] p-4">
                <label className="relative block">
                  ...
                  <input
                    ...
                    className="w-full rounded-md border border-[#cbd5e1] py-2 pl-10 pr-3 text-sm"
                  />
                </label>
              </div>
              {isLoading ? (
                <p className="grid min-h-96 place-items-center p-8 text-center text-sm text-[#667085]">
                  Loading teachers…
                </p>
              ) : visible.length ? (
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (pendingFolder) onDeleteFolder(pendingFolder.id);
...
function TeacherProfile({
  ...
  const createRequest = useMutation({
    mutationFn: () =>
      createTeacherRequisition(teacherId, {
        label,
        academicYear,
  ...
  return (
    <div className="mx-auto max-w-6xl px-1 py-6 sm:px-1.5 lg:px-2">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 text-sm font-semibold text-[#1f4e79]"
      >
        <ArrowLeft size={17} /> Back to teachers
      </button>
      <div className="mt-4 flex flex-col justify-between gap-4 border-b border-[#d9dee7] pb-5 sm:flex-row sm:items-end">
        <div className="flex items-center gap-3">
          <TeacherAvatar fullName={profile.fullName} size="large" />
          <div>
        ...
        <div className="flex gap-3">
          {profile.archivedAt ? (
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-md border border-[#b7bec8] px-3 py-2 text-sm font-semibold text-[#a6292f]"
            >
...
export function RequisitionHistory({
  ...
  return (
    <div className="mt-4">
      ...
      <div role="list" className="mt-4 grid gap-3">
        {visibleRequisitions.map((requisition) => (
          <article
            key={requisition.id}
            role="listitem"
            className="relative flex flex-col gap-3 rounded-lg border border-[#d9dee7] bg-white p-4 transition-colors hover:border-[#b9d0e5] hover:bg-[#f8fafc] sm:flex-row sm:items-center"
          >
            <button
              type="button"
              onClick={() => onOpen(requisition.id)}
              aria-label={`Open ${requisition.label}`}
              className="absolute inset-0 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1f4e79] focus:ring-offset-2"
            >
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <input
                      ...
                      className="min-w-0 flex-1 rounded-md border border-[#b7bec8] px-3 py-2 text-sm font-semibold"
                    />
                    {renaming ? (
...
function TeacherDocumentsCard({
  ...
  return (
    <section
      className={`${className} rounded-lg border border-[#d9dee7] bg-white p-5`}
    >
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div>
      ...
      {credential && documents.data ? (
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <a
            ...
            className="inline-flex items-center gap-2 rounded-md border border-[#b7bec8] px-3 py-2 text-sm font-semibold text-[#1f4e79]"
          >
            <Folder size={16} /> Open Google Drive folder
          </a>
          <button
            type="button"
...
export function ProfileOverview({
  ...
  const [draft, setDraft] = useState<
    Pick<Teacher, "fullName" | "email" | "phone" | "notes">
  >({
          ...
          >
            ...
            <div className="flex gap-3 md:col-span-2">
              ...
              <button
                ...
                disabled={saving}
                onClick={cancel}
                className="rounded-md border border-[#b7bec8] px-3 py-2 text-sm font-semibold"
              >
                Cancel
              </button>
            </div>
...
function ProfileDetail({
  ...
  return (
    <div className={className}>
      ...
      <dd
        className={`mt-1 whitespace-pre-wrap text-sm ${value ? "text-[#171717]" : "text-[#667085]"}`}
      >
        {value || emptyLabel}
      </dd>
    </div>
...
export function TeacherRequisitionEditor({
  ...
  const teacherBadge = (
    <div className="inline-flex items-center gap-2 rounded-md border border-[#b9d0e5] bg-[#f2f7fb] px-2.5 py-2 text-left">
      <TeacherAvatar fullName={teacherName} />
      <span>
        <span className="block text-xs font-medium uppercase tracking-wide text-[#667085]">
          Teacher
        </span>
        <span className="block text-sm font-semibold text-[#1f4e79]">
          {teacherName}
        </span>
      </span>
    </div>
  ...
  return (
    ...
    >
      <SectionEditorShell
        backLabel="Back to teacher profile"
        onBack={onBack}
        eyebrow={draft.academicYear}
        title={draft.label || "Untitled requisition"}
        titleControl={titleControl}
        subtitle="Teaching-recruitment request"
        titleMeta={<div className="mt-3">{teacherBadge}</div>}
        sections={[
        ...
        notice={
          validationMessage ? (
            <p
              role="alert"
              className="rounded-md border border-[#efc9cb] bg-[#fff5f5] px-3 py-2 text-sm text-[#8f1f25]"
            >
              {validationMessage}
            </p>
          ) : undefined
      ...
      >
        <section className="min-w-0 rounded-lg border border-[#d9dee7] bg-white p-5">
          {active === "details" ? (
            <>
              <div data-requisition-field="academic-year">
                <InputField
                  label="Academic year"
          ...
          {active === "courses" ? (
            <>
              <p className="rounded-md bg-[#eaf1f8] px-3 py-2 text-sm font-semibold text-[#1f4e79]">
...
function InlineRequisitionTitle({
  ...
  if (editing)
    return (
      <div data-requisition-field="requisition-title" className="max-w-md">
        ...
        <input
          ...
          className="h-10 w-full rounded-md border border-[#1f4e79] bg-white px-3 text-xl font-semibold text-[#171717] outline-none ring-2 ring-[#d7e5f3]"
        />
      </div>
    );
  return (
    <h2
      ...
      className="truncate font-semibold text-[#171717] transition-[font-size] duration-200 text-xl"
    >
      <button
        type="button"
        onClick={onEdit}
...
function RequisitionFieldAnchor({
  ...
  return <div data-requisition-field={target}>{children}</div>;
}
...
function ReviewDetail({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <dd className="mt-1 text-sm text-[#171717]">{value}</dd>
    </div>
  );
}
export function PhoneField({
  ...
  return (
    <label className="grid self-start gap-1 text-sm font-medium text-[#344054]">

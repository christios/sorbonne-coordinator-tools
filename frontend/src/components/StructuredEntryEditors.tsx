...
export function AssessmentItemsEditor({ value, outcomes, assessmentTypes = [], aiPolicies = [], onChange, ...history }: HistoryContext & { value: Record<string, unknown>; outcomes: Array<Record<string, string> & { id: string }>; assessmentTypes?: CatalogueEntry[]; aiPolicies?: CatalogueEntry[]; onChange: (value: Record<string, unknown>) => void }) {
  ...
  const update = (id: string, field: keyof AssessmentEntry, next: string | string[]) => onChange({ ...value, items: entries.map((entry) => entry.id === id ? { ...entry, [field]: next } : entry) });
  const remove = (id: string) => onChange({ ...value, items: entries.filter((entry) => entry.id !== id) });
  return <section><div><h4 className="text-sm font-semibold text-[#344054]">Summary of graded learning activities</h4><p className="mt-1 text-sm text-[#667085]">Add one assessment at a time and link it to the relevant course outcomes.</p></div><div className="mt-3 grid gap-3">{entries.map((entry, index) => <AssessmentCard key={entry.id} entry={entry} index={index} outcomes={outcomes} assessmentTypes={assessmentTypes} aiPolicies={aiPolicies} onChange={update} onRemove={remove} {...history} />)}</div>{entries.length === 0 ? <p className="mt-3 rounded-md border border-dashed border-[#d0d5dd] px-3 py-3 text-sm text-[#667085]">No assessments added.</p> : null}<AddEntryButton onClick={() => onChange({ ...value, items: [...entries, { id: crypto.randomUUID() }] })} label="Add assessment" /></section>;
}
...
function AssessmentCard({ entry, index, outcomes, assessmentTypes = [], aiPolicies = [], onChange, onRemove, ...history }: HistoryContext & { entry: AssessmentEntry; index: number; outcomes: Array<Record<string, string> & { id: string }>; assessmentTypes?: CatalogueEntry[]; aiPolicies?: CatalogueEntry[]; onChange: (id: string, field: keyof AssessmentEntry, value: string | string[]) => void; onRemove: (id: string) => void }) {

...
export function AssessmentItemsEditor({ value, outcomes, assessmentTypes = [], aiPolicies = [], onChange, ...history }: HistoryContext & { value: Record<string, unknown>; outcomes: Array<Record<string, string> & { id: string }>; assessmentTypes?: CatalogueEntry[]; aiPolicies?: CatalogueEntry[]; onChange: (value: Record<string, unknown>) => void }) {
  ...
  const update = (id: string, changes: Partial<AssessmentEntry>) => onChange({ ...value, items: entries.map((entry) => entry.id === id ? { ...entry, ...changes } : entry) });
  const remove = (id: string) => onChange({ ...value, items: entries.filter((entry) => entry.id !== id) });
  return <section><div className="flex flex-wrap items-start justify-between gap-3"><div><h4 className="text-sm font-semibold text-[#344054]">Summary of graded learning activities</h4><p className="mt-1 text-sm text-[#667085]">Add one assessment at a time and link it to the relevant course outcomes.</p></div><fieldset className="shrink-0"><legend className="text-sm font-medium text-[#344054]">Show each assessment by</legend><div className="mt-1 inline-flex rounded-md border border-[#b7bec8] bg-white p-0.5">{([["date","Date"],["week","Week"]] as const).map(([mode,modeLabel]) => <button key={mode} type="button" aria-pressed={scheduleBy === mode} onClick={() => onChange({ ...value, scheduleBy: mode })} className={`rounded px-3 py-1 text-sm font-semibold ${scheduleBy === mode ? "bg-[#1f4e79] text-white" : "text-[#1f4e79] hover:bg-[#f2f7fb]"}`}>{modeLabel}</button>)}</div></fieldset></div><div className="mt-3 grid gap-3">{entries.map((entry, index) => <AssessmentCard key={entry.id} entry={entry} index={index} outcomes={outcomes} assessmentTypes={assessmentTypes} aiPolicies={aiPolicies} scheduleBy={scheduleBy} onChange={update} onRemove={remove} {...history} />)}</div>{entries.length === 0 ? <p className="mt-3 rounded-md border border-dashed border-[#d0d5dd] px-3 py-3 text-sm text-[#667085]">No assessments added.</p> : null}<AddEntryButton onClick={() => onChange({ ...value, items: [...entries, { id: crypto.randomUUID() }] })} label="Add assessment" /></section>;
}
...
function AssessmentCard({ entry, index, outcomes, assessmentTypes = [], aiPolicies = [], scheduleBy = "date", onChange, onRemove, ...history }: HistoryContext & { entry: AssessmentEntry; index: number; scheduleBy?: "date" | "week"; outcomes: Array<Record<string, string> & { id: string }>; assessmentTypes?: CatalogueEntry[]; aiPolicies?: CatalogueEntry[]; onChange: (id: string, changes: Partial<AssessmentEntry>) => void; onRemove: (id: string) => void }) {
  ...
  return <article className="rounded-lg border border-[#d9dee7] bg-[#fdfdfd]"><div className="flex items-center gap-3 p-3"><button type="button" onClick={() => setOpen((current) => !current)} className="flex min-w-0 flex-1 items-center gap-2 text-left"><ChevronDown size={17} className={`shrink-0 text-[#667085] transition-transform ${open ? "rotate-180" : ""}`} /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-[#344054]">{stringValue(entry.name) || stringValue(entry.type) || `Assessment ${index + 1}`}</span><span className="mt-0.5 block truncate text-sm font-normal text-[#667085]">{assessmentSummary(entry, outcomes, scheduleBy) || "Not filled in yet"}</span></span></button><button type="button" onClick={() => onRemove(entry.id)} className="rounded p-1 text-[#a6292f] hover:bg-[#fff1f2]" aria-label={`Remove assessment ${index + 1}`}><X size={17} /></button></div>{open ? <div className="grid gap-4 border-t border-[#e5e7eb] p-4"><div className="grid gap-4 sm:grid-cols-2">{scheduleBy === "week" ? <EntryInput label="Week" value={stringValue(entry.week)} onChange={(next) => set("week", next)} type="number" min={1} step={1} field={{ path: `${path}.week`, label: `Assessment ${index + 1} · Week` }} {...history} /> : <EntryInput label="Assessment date" value={stringValue(entry.date)} onChange={(next) => set("date", next)} type="date" field={{ path: `${path}.date`, label: `Assessment ${index + 1} · Assessment date` }} {...history} />}{typeOptions.length ? <label className="grid gap-1 text-sm font-medium text-[#344054]">Assessment type<SelectMenu label={`Assessment ${index + 1} type`} value={stringValue(entry.assessmentTypeId)} onChange={(next) => { const chosen = assessmentTypes.find((item) => item.id === next); onChange(entry.id, { assessmentTypeId: next, type: chosen?.label ?? "" }); }} options={typeOptions} placeholder="Select assessment type" /></label> : <EntryInput label="Assessment type" value={stringValue(entry.type)} onChange={(next) => set("type", next)} field={{ path: `${path}.type`, label: `Assessment ${index + 1} · Assessment type` }} {...history} />}<EntryInput label="Name for this activity (optional)" value={stringValue(entry.name)} onChange={(next) => set("name", next)} field={{ path: `${path}.name`, label: `Assessment ${index + 1} · Name` }} {...history} /><EntryInput label="Weight (%)" value={stringValue(entry.weight)} onChange={(next) => set("weight", next)} type="number" min={0} max={100} step={1} field={{ path: `${path}.weight`, label: `Assessment ${index + 1} · Weight` }} {...history} /><label className="grid gap-1 text-sm font-medium text-[#344054]">Applicable AI policy<SelectMenu label={`Assessment ${index + 1} AI policy`} value={stringValue(entry.aiPolicy ?? entry.ai)} onChange={(next) => onChange(entry.id, { aiPolicy: next, ai: next })} options={(aiPolicies.length ? aiPolicies.map((item) => item.label) : AI_POLICIES).map((policy) => ({ value: policy, label: policy }))} trailing={<FieldHistoryControl syllabusId={history.syllabusId} revision={history.revision} field={{ path: `${path}.aiPolicy`, label: `Assessment ${index + 1} · AI policy` }} onOpenSidebar={history.onOpenHistory} />} /></label></div><PloAlignmentField label="CLOs assessed" pickerLabel={`Add CLO assessed by assessment ${index + 1}`} emptyText="No course outcomes selected yet." addText="Add CLO assessed" value={selectedIds.map((id) => cloLabel(outcomes, id)).filter(Boolean).join("\n")} onChange={(next) => { const labels = next.split("\n").filter(Boolean); const ids = labels.map((label) => outcomes.find((outcome) => cloLabel(outcomes, outcome.id) === label)?.id).filter((id): id is string => Boolean(id)); onChange(entry.id, { cloIds: ids, clos: labels.join("\n") }); }} options={outcomes.map((outcome) => ({ value: cloLabel(outcomes, outcome.id), label: cloLabel(outcomes, outcome.id) }))} history={<FieldHistoryControl syllabusId={history.syllabusId} revision={history.revision} field={{ path: `${path}.cloIds`, label: `Assessment ${index + 1} · CLOs assessed` }} onOpenSidebar={history.onOpenHistory} placement="label" />} /></div> : null}</article>;
}
...
function EntryInput({ label, value, onChange, field, syllabusId, revision, onOpenHistory, multiline = false, type = "text", min, max, step }: HistoryContext & { label: string; value: string; onChange: (value: string) => void; field: HistoryField; multiline?: boolean; type?: string; min?: number; max?: number; step?: number }) {
  if (type === "date") return <DateField label={label} value={value} onChange={onChange} trailing={<FieldHistoryControl syllabusId={syllabusId} revision={revision} field={field} onOpenSidebar={onOpenHistory} placement="center" />} />;
  return <HistoryTextField label={label} value={value} onChange={onChange} multiline={multiline} minRows={3} type={type} min={min} max={max} step={step} history={{ field, onOpenHistory }} />;
}

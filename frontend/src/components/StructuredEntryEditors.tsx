...
export function AssessmentItemsEditor({ value, outcomes, assessmentTypes = [], onChange, ...history }: HistoryContext & { value: Record<string, unknown>; outcomes: Array<Record<string, string> & { id: string }>; assessmentTypes?: CatalogueEntry[]; onChange: (value: Record<string, unknown>) => void }) {
  const entries = assessmentEntries(value.items);
  const update = (id: string, field: keyof AssessmentEntry, next: string | string[]) => onChange({ ...value, items: entries.map((entry) => entry.id === id ? { ...entry, [field]: next } : entry) });
  const remove = (id: string) => onChange({ ...value, items: entries.filter((entry) => entry.id !== id) });
  return <section><div><h4 className="text-sm font-semibold text-[#344054]">Summary of graded learning activities</h4><p className="mt-1 text-sm text-[#667085]">Add one assessment at a time and link it to the relevant course outcomes.</p></div><div className="mt-3 grid gap-3">{entries.map((entry, index) => <AssessmentCard key={entry.id} entry={entry} index={index} outcomes={outcomes} assessmentTypes={assessmentTypes} onChange={update} onRemove={remove} {...history} />)}</div>{entries.length === 0 ? <p className="mt-3 rounded-md border border-dashed border-[#d0d5dd] px-3 py-3 text-sm text-[#667085]">No assessments added.</p> : null}<AddEntryButton onClick={() => onChange({ ...value, items: [...entries, { id: crypto.randomUUID() }] })} label="Add assessment" /></section>;
}
...
export function AssessmentGradingEditor({ value, onChange, ...history }: HistoryContext & { value: Record<string, unknown>; onChange: (value: Record<string, unknown>) => void }) {
  return <div className="grid gap-7"><RubricEditor value={value.rubrics} onChange={(rubrics) => onChange({ ...value, rubrics })} {...history} /><NarrativeField label="Assessment methodologies" value={stringValue(value.methodologies)} onChange={(methodologies) => onChange({ ...value, methodologies })} field={{ path: "assessment.methodologies", label: "Assessment methodologies" }} {...history} /><NarrativeField label="Late submission policy" value={stringValue(value.lateSubmissionPolicy)} onChange={(lateSubmissionPolicy) => onChange({ ...value, lateSubmissionPolicy })} field={{ path: "assessment.lateSubmissionPolicy", label: "Late submission policy" }} {...history} /></div>;
}

function AssessmentCard({ entry, index, outcomes, assessmentTypes = [], onChange, onRemove, ...history }: HistoryContext & { entry: AssessmentEntry; index: number; outcomes: Array<Record<string, string> & { id: string }>; assessmentTypes?: CatalogueEntry[]; onChange: (id: string, field: keyof AssessmentEntry, value: string | string[]) => void; onRemove: (id: string) => void }) {

...
export function AssessmentGradingEditor({ value, rubricPresets = [], ...history }: HistoryContext & { value: Record<string, unknown>; rubricPresets?: CatalogueEntry[]; onChange?: (value: Record<string, unknown>) => void }) {
  void history;
  ...
  const shown = rubricPresets.filter((preset) => used.includes(stringValue(preset.payload.assessmentTypeId)));
  return <section><div><h4 className="text-sm font-semibold text-[#344054]">Grading rubrics</h4><p className="mt-1 text-sm text-[#667085]">One rubric per assessment type used in this course. They are maintained in the catalogue and appear here automatically.</p></div>{shown.length ? <div className="mt-4 grid gap-4">{shown.map((preset) => <RubricPresetTable key={preset.id} preset={preset} />)}</div> : <p className="mt-3 rounded-md border border-dashed border-[#d0d5dd] px-3 py-3 text-sm text-[#667085]">Choose an assessment type for each graded activity and its rubric will appear here.</p>}</section>;
}

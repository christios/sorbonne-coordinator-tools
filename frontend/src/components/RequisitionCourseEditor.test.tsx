...
describe("RequisitionCourseEditor", () => {
  it("adds one expanded course card instead of an editable table row", () => {
    ...
    expect(courses[0]).toMatchObject({ subjectCode: "", courseNumber: "", level: "", title: "", hours: "" });
  });
  ...
  it("adds a card before offering the list picker or manual fields", () => {
    ...
    expect(screen.queryByRole("combobox", { name: "Choose from course list" })).toBeNull();
  ...
  it("keeps complete courses compact until opened", () => {
    render(
      <RequisitionCourseEditor
        courses={[{ id: "mechanics", subjectCode: "PHY", courseNumber: "101", level: "L1", title: "Mechanics", hours: "24" }]}
        onChange={vi.fn()}
      />,
    );

    expect(screen.getByRole("button", { name: "Expand course: Mechanics" })).toBeTruthy();
  ...
  it("adds a snapshot of the selected active catalogue course by CRN", () => {
    ...
    render(
      <RequisitionCourseEditor
        courses={[{ id: "draft", subjectCode: "", courseNumber: "", level: "", title: "", hours: "" }]}
        onChange={onChange}
        catalogueCourses={[{ id: "catalogue-physics", crn: "21939", term: "262710", courseCode: "PHY-101", courseTitle: "Physics", sequence: "1", credit: "4", department: "PHY", level: "L1", college: "P4", contactHours: "30", isObsolete: false, importedAt: "2026-07-24T00:00:00Z", obsoleteAt: null }]}
      />,
    );
  ...
  it("uses an in-app confirmation before removing a course", () => {
    const onChange = vi.fn();
    render(<RequisitionCourseEditor courses={[{ id: "physics", subjectCode: "PHY", courseNumber: "101", level: "L1", title: "Physics", hours: "24" }]} onChange={onChange} />);

    fireEvent.click(screen.getByRole("button", { name: "Remove course: Physics" }));

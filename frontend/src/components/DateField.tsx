    ...
    <div className="space-y-1">
      ...
      <div className="relative">
        ...
        <Popover.Trigger asChild>
          ...
          >
            <CalendarDays size={16} className="shrink-0 text-[#1f4e79]" aria-hidden="true" />
            <span className={selected ? "truncate text-[#171717]" : "truncate text-[#667085]"}>{selected ? formatDate(selected) : "Select date"}</span>
          </button>
        </Popover.Trigger>
        <Popover.Portal>
          <Popover.Content
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <button type="button" onClick={() => setMonth((current) => new Date(current.getFullYear(), current.getMonth() + 1, 1))} className="rounded-md p-2 text-[#1f4e79] hover:bg-[#f2f7fb]" aria-label="Next month"><ChevronRight size={17} /></button>
            </div>
            <div className="mt-3 grid grid-cols-7 gap-1 text-center text-xs font-medium text-[#667085]">{WEEKDAYS.map((weekday) => <span key={weekday}>{weekday}</span>)}</div>
            <div role="grid" aria-label={`${label} calendar`} className="mt-1 grid grid-cols-7 gap-1">{days.map((day, index) => day ? <button key={toIsoDate(day)} type="button" role="gridcell" onClick={() => choose(day)} aria-label={`Select ${formatDate(day)}`} aria-pressed={sameDate(day, selected)} className={`h-8 rounded-md text-sm transition-colors ${sameDate(day, selected) ? "bg-[#1f4e79] font-semibold text-white" : sameDate(day, new Date()) ? "border border-[#b9d0e5] text-[#1f4e79] hover:bg-[#f2f7fb]" : "text-[#344054] hover:bg-[#f2f7fb]"}`}>{day.getDate()}</button> : <span key={`empty-${index}`} aria-hidden="true" className="h-8" />)}</div>
            <div className="mt-3 flex items-center justify-between border-t border-[#e5e7eb] pt-3"><button type="button" onClick={() => { setMonth(startOfMonth(new Date())); }} className="text-sm font-semibold text-[#1f4e79] hover:underline">Today</button>{selected ? <button type="button" onClick={() => { onChange(""); setOpen(false); }} className="inline-flex items-center gap-1 text-sm font-semibold text-[#a6292f] hover:underline"><X size={15} /> Clear</button> : null}</div>
          </Popover.Content>
        </Popover.Portal>
        </Popover.Root>
        {trailing}

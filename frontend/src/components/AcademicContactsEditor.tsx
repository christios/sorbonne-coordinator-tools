...
function InstructorCard({ entry, index, people, options, courseTeachers = [], onChange, onRemove, ...history }: HistoryContext & { entry: InstructorEntry; index: number; people: CatalogueEntry[]; options: SelectOption[]; courseTeachers?: string[]; onChange: (value: InstructorEntry) => void; onRemove?: () => void }) {
  ...
  return <article className="rounded-lg border border-[#d9dee7] bg-[#fdfdfd] p-4">
    ...
    <label className="grid gap-1 text-sm font-medium text-[#344054]"><span>From People directory <span className="font-normal text-[#667085]">(optional)</span></span><SelectMenu label={`Instructor ${index + 1} from People directory`} value={stringValue(entry.personId)} onChange={(personId) => onChange({ ...entry, personId: personId || undefined })} placeholder={options.length ? "Choose an instructor" : "No instructors in the directory yet"} searchable options={[{ value: "", label: "Enter contact details manually" }, ...options]} />{options.length ? null : <span className="text-xs font-normal text-[#667085]">Nobody is listed as an instructor yet. An administrator adds them in Manage catalogues → People.</span>}</label>
    {courseTeachers.length && !person ? <label className="mt-3 grid gap-1 text-sm font-medium text-[#344054]"><span>From this course\u2019s timetable <span className="font-normal text-[#667085]">(optional)</span></span><SelectMenu label={`Instructor ${index + 1} from the timetable`} value="" onChange={(name) => onChange({ ...entry, Name: name })} placeholder="Choose whoever teaches a section" options={courseTeachers.map((name) => ({ value: name, label: name }))} /></label> : null}{person ? <div className="mt-3"><LivePersonCard person={person} /></div> : <div className="mt-3 grid gap-4">

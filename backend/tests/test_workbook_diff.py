...
def test_a_course_the_block_does_not_teach_yet_is_its_own_decision():
    incoming = workbook(
        courses=[ImportedCourse(code="MATH001"), ImportedCourse(code="MATH009", name="Algebra")]
    )
...
def test_a_student_already_where_the_workbook_puts_them_is_not_a_decision():
    report = diff_assignments(held={"A1": {"CM": "1"}}, incoming={"A1": {"CM": "1"}}, groups=GROUPS)
    assert report["rows"] == []
    assert report["unchanged"] == 1


def test_a_student_nobody_has_placed_reads_as_placed():
    report = diff_assignments(held={}, incoming={"A1": {"CM": "1"}}, groups=GROUPS)
    (row,) = report["rows"]
...
def test_a_student_the_workbook_moves_says_where_from_and_to():
    report = diff_assignments(held={"A1": {"CM": "1"}}, incoming={"A1": {"CM": "2"}}, groups=GROUPS)
    (row,) = report["rows"]
...
def test_a_group_the_catalogue_does_not_have_is_named_not_skipped_quietly():
    report = diff_assignments(held={}, incoming={"A1": {"CM": "99"}}, groups=GROUPS)
    assert report["rows"] == []
    assert report["unknownGroups"] == ["CM 99"]


def test_a_placement_the_workbook_no_longer_mentions_is_left_alone():
    report = diff_assignments(held={"A1": {"CM": "1", "TD": "3"}}, incoming={"A1": {"CM": "1"}}, groups=GROUPS)
    assert report["rows"] == []
